...
export default function AboutPage() {
  const values = [
    ...
    {
      icon: <ShieldCheck className="h-6 w-6 text-text-primary" />,
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-background text-text-primary">
        {/* Cinematic Header */}
        <section className="relative pt-32 pb-20 overflow-hidden text-center bg-surface border-b border-border-custom">
          <div className="max-w-4xl mx-auto px-4 md:px-8 space-y-4 relative z-10">
            <span className="text-[10px] tracking-[0.25em] font-mono text-text-muted uppercase block">Our Heritage</span>
            <h1 className="text-4xl md:text-7xl font-display font-light italic text-text-primary leading-none">
              The Art of Fine Dining
            </h1>
            <div className="h-[1px] w-20 bg-border-custom mx-auto mt-4" />
            <p className="text-xs sm:text-sm text-text-secondary leading-relaxed max-w-2xl mx-auto font-sans font-light">
        ...
        <section className="py-16 sm:py-24 max-w-7xl mx-auto px-4 md:px-8 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-7 space-y-6 text-left">
            <h2 className="font-display italic text-3xl md:text-5xl font-light leading-tight text-text-primary">
              A Culinary Sanctuary in <span className="underline decoration-border-custom">Secunderabad</span>
            </h2>
          ...
          <div className="lg:col-span-5 relative h-[380px] lg:h-[480px] rounded-2xl overflow-hidden border border-border-custom shadow-sm bg-surface">
            <Image
        ...
        <section className="py-20 bg-surface border-y border-border-custom">
          <div className="max-w-7xl mx-auto px-4 md:px-8">
            <div className="text-center max-w-2xl mx-auto space-y-3 mb-16">
              <span className="text-[10px] tracking-[0.25em] font-mono text-text-muted uppercase block">Our Guiding Pillars</span>
              <h2 className="text-3xl sm:text-4xl font-display font-light italic text-text-primary">Uncompromising Dining Philosophy</h2>
              <div className="h-[1px] w-16 bg-border-custom mx-auto mt-2" />
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {values.map((v, i) => (
                <div key={i} className="bg-surface-2 p-6 rounded-2xl space-y-4 border border-border-custom flex flex-col items-start text-left">
                  <div className="p-3 rounded-xl bg-surface border border-border-custom">{v.icon}</div>
                  <h3 className="font-display italic text-lg font-semibold text-text-primary">{v.title}</h3>
                  <p className="text-xs text-text-secondary leading-relaxed font-sans font-light">{v.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-20 bg-background">
          <div className="max-w-7xl mx-auto px-4 md:px-8">
            <div className="text-center max-w-2xl mx-auto space-y-3 mb-16">
              <span className="text-[10px] tracking-[0.25em] font-mono text-text-muted uppercase block">Visual Poetry</span>
              <h2 className="text-3xl sm:text-4xl font-display font-light italic text-text-primary">Inside Our Culinary Sanctuary</h2>
              <div className="h-[1px] w-16 bg-border-custom mx-auto mt-2" />
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {galleryImages.map((img, idx) => (
                <div key={idx} className="relative h-64 w-full rounded-2xl overflow-hidden group border border-border-custom bg-surface">
                  <Image
                    src={img.url}
                    alt={img.title}
                    fill
                  ...
                  <div className="absolute bottom-4 left-4 right-4 text-left">
                    <h3 className="text-[10px] font-sans font-semibold text-text-primary uppercase tracking-widest">{img.title}</h3>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section className="py-20 bg-surface border-t border-border-custom relative overflow-hidden">
          <div className="max-w-4xl mx-auto px-4 md:px-8 text-center space-y-6">
            <Sparkles className="h-7 w-7 text-text-primary mx-auto animate-pulse-slow" />
            <blockquote className="font-display text-lg sm:text-2xl leading-relaxed italic text-text-primary font-light max-w-2xl mx-auto">
            ...
            <div className="space-y-1">
              <cite className="not-italic text-sm font-semibold uppercase tracking-wider text-text-primary font-sans">
                Executive Chef Team
              </cite>
              <p className="text-[9px] font-mono text-text-muted uppercase">Cafe The Multi Cuisine Restaurant</p>
            </div>
          </div>
        </section>
      </div>
